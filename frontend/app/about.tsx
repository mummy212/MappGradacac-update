...
export default function AboutScreen() {
  ...
  const handleDonate = (amount: number) => {
    if (!paypalLink) {
      Alert.alert('PayPal', t('about', 'paypalNotSetMsg'));
      return;
    }
    let url = paypalLink;
    ...
    Linking.openURL(url).catch(() => Alert.alert(t('common','error'), t('about','paypalErrorMsg')));
  };

  const handleCustomDonate = () => {
    const amount = parseFloat(customAmount);
    if (!amount || amount <= 0) { Alert.alert(t('common','error'), t('about','customAmountError')); return; }
    handleDonate(amount);
  };
  ...
  return (
    <View testID="about-screen" style={[s.container, { paddingTop: insets.top }]}>
      ...
      <View style={s.header}>
        ...
        <Text style={s.headerTitle}>{t('about', 'header')}</Text>
        <View style={{ width: 40 }} />
      </View>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={s.scroll}>
        ...
        {showHiddenMenu && (
          <View testID="hidden-menu" style={s.hiddenMenu}>
            ...
            <TouchableOpacity testID="hidden-biz-btn" style={s.hiddenBtn} onPress={() => { setShowHiddenMenu(false); router.push('/business'); }}>
              ...
              <Text style={s.hiddenBtnTxt}>{t('about', 'businessPanel') || 'Biznis Panel'}</Text>
            </TouchableOpacity>
            <TouchableOpacity testID="hidden-close-btn" style={s.hiddenClose} onPress={() => setShowHiddenMenu(false)}>
              <Text style={s.hiddenCloseTxt}>{t('common', 'close')}</Text>
            </TouchableOpacity>
          </View>
        )}
        ...
        <View style={s.section}>
          <Text style={s.sectionTitle}>{t('about', 'featuresTitle')}</Text>
          <FeatureRow icon="map-outline" text={t('about', 'feature1')} />
          <FeatureRow icon="search-outline" text={t('about', 'feature2')} />
          <FeatureRow icon="star-outline" text={t('about', 'feature3')} />
          <FeatureRow icon="navigate-outline" text={t('about', 'feature4')} />
          <FeatureRow icon="notifications-outline" text={t('about', 'feature5')} />
          <FeatureRow icon="call-outline" text={t('about', 'feature6')} />
          <FeatureRow icon="qr-code-outline" text={t('about', 'feature7')} />
          <FeatureRow icon="pricetag-outline" text={t('about', 'feature8')} />
        </View>
        ...
        <View style={s.donateSection}>
          <View style={s.donateHeader}>
            ...
            <Text style={s.donateTitle}>{t('about', 'donateTitle')}</Text>
          </View>
          <Text style={s.donateDesc}>{t('about', 'donateDesc')}</Text>
          ...
          <View style={s.customRow}>
            <View style={s.customInputWrap}>
              <TextInput testID="custom-amount-input" style={s.customInput} value={customAmount}
                ...
                placeholder={t('about', 'customAmountPlaceholder')} placeholderTextColor={colors.textSecondary} keyboardType="numeric" />
              <Text style={s.customSymbol}>{symbol}</Text>
            </View>
          </View>
          <TouchableOpacity testID="donate-btn" style={s.donateBtn}
            onPress={() => { if (customAmount) handleCustomDonate(); else if (selectedAmount) handleDonate(selectedAmount); else Alert.alert(t('common','selectAmount')); }}>
            <Ionicons name="logo-paypal" size={22} color="#fff" />
            <Text style={s.donateBtnText}>{t('about', 'donateBtn')}</Text>
          </TouchableOpacity>
          <Text style={s.secureTxt}>
            <Ionicons name="lock-closed" size={12} color={colors.textSecondary} /> {t('about', 'securePaypal')}
          </Text>
        </View>

        {/* Privacy Policy (Required for App Store) */}
        <View style={s.section}>
          <Text style={s.sectionTitle}>{t('about', 'privacyTitle')}</Text>
          <Text style={s.privacyText}>{t('about', 'privacyIntro')}</Text>
          <View style={s.privacyItem}><Ionicons name="location-outline" size={16} color={colors.primary} /><Text style={s.privacyItemText}>{t('about', 'privacyLocation')}</Text></View>
          <View style={s.privacyItem}><Ionicons name="camera-outline" size={16} color={colors.primary} /><Text style={s.privacyItemText}>{t('about', 'privacyCamera')}</Text></View>
          <View style={s.privacyItem}><Ionicons name="star-outline" size={16} color={colors.primary} /><Text style={s.privacyItemText}>{t('about', 'privacyReviews')}</Text></View>
          <View style={s.privacyItem}><Ionicons name="notifications-outline" size={16} color={colors.primary} /><Text style={s.privacyItemText}>{t('about', 'privacyNotifications')}</Text></View>
          <Text style={s.privacyText}>{t('about', 'privacyFooter')}</Text>
        </View>

        {/* Terms of Use (Required for App Store) */}
        <View style={s.section}>
          <Text style={s.sectionTitle}>{t('about', 'termsTitle')}</Text>
          <Text style={s.privacyText}>{t('about', 'termsText')}</Text>
        </View>
        ...
        <View style={s.section}>
          ...
          <Text style={s.sectionTitle}>{t('about', 'reportTitle')}</Text>
          <Text style={s.privacyText}>{t('about', 'reportText')}</Text>
          <TouchableOpacity testID="report-content-btn" style={s.reportBtn} onPress={() => Linking.openURL(`mailto:${contactEmail}?subject=Prijava neprimjerenog sadržaja`)}>
            ...
            <Text style={s.reportBtnTxt}>{t('about', 'reportBtn')}</Text>
          </TouchableOpacity>
        </View>
        ...
        <Text style={s.footer}>{t('about', 'footer')}</Text>
